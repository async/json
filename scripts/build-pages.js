...
const site = {
  ...
  repo: "json",
  stage: "Alpha",
  description: "JSON file and folder database engine with sidecar state, queries, version history, and optional RedisJSON storage.",
  lead: "Import a JSON file like a tiny database, then graduate through @async/db when schemas, APIs, readers, and store portability matter.",
  quickstart: "pnpm add @async/json\n\nimport json from '@async/json';\n\nconst users = await json('./users.json');\nawait users.find({ where: { role: 'admin' } });"
};
...
await writeFile(join(outDir, "api-surface.html"), layout({
  ...
  body: `<div class="markdown">${renderMarkdown(apiSurface)}</div>`,
  rootPrefix: ""
}));
